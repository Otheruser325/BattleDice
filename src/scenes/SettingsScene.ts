...
export class SettingsScene extends Phaser.Scene {
  ...
  private subModalCloseHandler: (() => void) | null = null;
  ...
  private async openChangelogModal() {
    ...
    const overlay = this.add.rectangle(width / 2, height / 2, width, height, 0x02080d, 0.72)
      .setInteractive()
      .setDepth(70);
    ...
    const onWheel = (_pointer: Phaser.Input.Pointer, _gameObjects: Phaser.GameObjects.GameObject[], _deltaX: number, deltaY: number) => {
      if (contentHeightActual > contentHeight) {
    ...
    const close = () => {
      ...
      this.subModalOpen = false;
      this.subModalCloseHandler = null;
      this.setModalInteractivity(true);
    };
    this.subModalCloseHandler = close;
