...
export class CasinoScene extends Phaser.Scene {
  ...
  private openChests(type: ChestType, openCount: number, isAll: boolean) {
    ...
    const merged = new Map<string, { title: string; rarity: string; copies: number; isNew: boolean }>();
    for (let i = 0; i < openCount; i++) {
      const reward = this.rollChestReward(type);
      if (!reward) continue;
      const current = merged.get(reward.typeId);
      if (current) merged.set(reward.typeId, { ...current, copies: current.copies + reward.copies, isNew: current.isNew || reward.isNew });
      else merged.set(reward.typeId, { title: reward.title, rarity: reward.rarity, copies: reward.copies, isNew: reward.isNew });
    }
    const entries = [...merged.entries()]
      .map(([typeId, data]) => ({ typeId, ...data }))
      .sort((a, b) => (this.rarityRank[a.rarity] - this.rarityRank[b.rarity]) || a.title.localeCompare(b.title));
  ...
  private showRewardsModal(type: ChestType, entries: Array<{ typeId: string; title: string; rarity: string; copies: number; isNew: boolean }>) {
    const { width, height } = this.scale;
    ...
    const title = this.add.text(width / 2, height / 2 - 250, `${type} Rewards`, { fontFamily: 'Orbitron', fontSize: '28px', color: PALETTE.accent }).setOrigin(0.5);
    const closeBtn = this.add.text(width / 2, height / 2 + 252, 'Close', { fontFamily: 'Orbitron', fontSize: '12px', color: PALETTE.textMuted, backgroundColor: '#173247', padding: { left: 10, right: 10, top: 5, bottom: 5 } }).setOrigin(0.5).setInteractive({ useHandCursor: true });
...
+${entry.copies} copies${entry.isNew ? '  NEW' : ''}`, { fontFamily: 'Orbitron', fontSize: '12px', color: PALETTE.text, lineSpacing: 4 });
    ...
    const close = () => { this.input.off('wheel', wheelHandler); [overlay, panel, title, closeBtn, container, mask].forEach((o) => o.destroy()); };
    closeBtn.on('pointerdown', close); overlay.on('pointerdown', close);
  }
