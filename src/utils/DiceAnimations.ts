...
export async function animateDiceRoll(scene: Phaser.Scene & {
...
export function animateTranscendenceBeamFx(scene: Phaser.Scene, attackerX: number, attackerY: number, targetGridX: number, rowY: number, targetX: number, targetY: number, boardWidth: number) {
    const g = scene.add.graphics().setDepth(260);
